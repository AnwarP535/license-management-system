...
A comprehensive license management system with React frontend and NestJS backend, supporting both web applications and mobile SDK integrations.

## Project Structure

```
license-management-system/
├── license-backend/      # NestJS backend with PostgreSQL
│   ├── src/
...
│   └── Dockerfile
│
├── license-client/        # React frontend with MUI
│   ├── src/
...
2. Edit `.env` file if needed (defaults work for development):
```bash
...
```bash
cp .env.example .env
# Edit .env: Change DB_HOST from 'postgres' to 'localhost'
```

4. Start PostgreSQL (using Docker/Podman):
```bash
docker run --name license-postgres \
  -e POSTGRES_PASSWORD=postgres \
  -e POSTGRES_DB=license_management \
